...
# 統計追蹤
global total_attacks_detected = 0;
global attack_types: set[string] = set();
global attacker_ips: set[addr] = set();

# === HTTP 暴力破解檢測 ===
event http_request(c: connection, method: string, original_URI: string,
                   unescaped_URI: string, version: string)
{
    # 檢查是否為登入請求（不檢查方法，因為可能是 OST）
                                          ...
                                          start_addr: count, quantity: count)
{
    ...
    if (orig == 192.168.10.51)
    {
        ...
        if (orig !in attack_stages)
            attack_stages[orig] = set();
        add attack_stages[orig]["modbus_scan"];
    }
    ...
    if (orig == 192.168.10.51)
    {
        NOTICE([
            ...
            $msg=fmt("Modbus 讀取線圈: 跳板主機 %s -> %s:%s, 起始: %d, 數量: %d", 
                    orig, c$id$resp_h, c$id$resp_p, start_addr, quantity),
            $conn=c,
            $identifier=fmt("%s-read-coils", orig)
        ]);
    }
}
...
event zeek_done()
{
    ...
    # 攻擊總結
    local attack_summary = "\n\n=== ICS 攻擊檢測總結 ===\n";
    attack_summary = fmt("%s總共檢測到攻擊事件: %d\n", attack_summary, total_attacks_detected);
    attack_summary = fmt("%s攻擊類型數量: %d\n", attack_summary, |attack_types|);
    attack_summary = fmt("%s涉及的攻擊者 IP: %s\n\n", attack_summary, fmt("%s", attacker_ips));
    
    # HTTP 暴力破解統計
    if (192.168.1.100 in http_login_attempts)
    {
        attack_summary = fmt("%s[HTTP 暴力破解]\n", attack_summary);
        attack_summary = fmt("%s  - 攻擊者: %s\n", attack_summary, 192.168.1.100);
        attack_summary = fmt("%s  - 登入嘗試次數: %d\n\n", attack_summary, http_login_attempts[192.168.1.100]);
    }
    
    # SSH 橫向移動統計
    local ssh_count = 0;
    for (src in ssh_connections)
    {
        ++ssh_count;
        attack_summary = fmt("%s[SSH 橫向移動 #%d]\n", attack_summary, ssh_count);
        attack_summary = fmt("%s  - 來源: %s\n", attack_summary, src);
        attack_summary = fmt("%s  - 目標: %s\n\n", attack_summary, fmt("%s", ssh_connections[src]));
    }
    
    # Modbus 攻擊統計
    if (192.168.10.51 in register_scan_patterns)
    {
        attack_summary = fmt("%s[Modbus 攻擊活動]\n", attack_summary);
        attack_summary = fmt("%s  - 攻擊者: %s\n", attack_summary, 192.168.10.51);
        attack_summary = fmt("%s  - 掃描的暫存器區域數: %d\n", attack_summary, |register_scan_patterns[192.168.10.51]|);
        
        if (192.168.10.51 in attack_stages)
        {
            attack_summary = fmt("%s  - 執行的攻擊類型: ", attack_summary);
            local first = T;
            for (stage in attack_stages[192.168.10.51])
            {
                if (first)
                {
                    attack_summary = fmt("%s%s", attack_summary, stage);
                    first = F;
                }
                else
                    attack_summary = fmt("%s, %s", attack_summary, stage);
            }
            attack_summary = fmt("%s\n", attack_summary);
        }
        attack_summary = fmt("%s\n", attack_summary);
    }
    
    # 完整攻擊鏈分析
    attack_summary = fmt("%s[攻擊鏈分析]\n", attack_summary);
    local chain_detected = F;
    for (attacker in attack_stages)
    {
        if (|attack_stages[attacker]| >= 2)
        {
            chain_detected = T;
            attack_summary = fmt("%s  - %s: ", attack_summary, attacker);
            local first2 = T;
    ...
    if (!chain_detected)
        attack_summary = fmt("%s  - 未檢測到多階段攻擊鏈\n", attack_summary);
    
    attack_summary = fmt("%s\n=========================\n", attack_summary);
    
    # 輸出總結
    print attack_summary;  # 輸出到 stdout
    Reporter::info(attack_summary);  # 輸出到 reporter.log
}
...
hook Notice::policy(n: Notice::Info)
{
    ...
    add attack_types[fmt("%s", n$note)];
    
    if (n?$conn && n$conn?$id && n$conn$id?$orig_h)
        add attacker_ips[n$conn$id$orig_h];
}